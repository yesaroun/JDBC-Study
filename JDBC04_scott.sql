...
SELECT SID, NAME, ENG, MAT, TOT, AVG, RANK
  FROM
(
    SELECT SID, NAME, KOR, ENG, MAT
         , (KOR + ENG + MAT) AS TOT
...
WHERE NAME = '이시우';
--==>> 한줄 구성
SELECT SID, NAME, ENG, MAT, TOT, AVG, RANK  FROM (    SELECT SID, NAME, KOR, ENG, MAT , (KOR + ENG + MAT) AS TOT , (KOR + ENG + MAT)/3 AS AVG , RANK() OVER(ORDER BY (KOR+ENG+MAT) DESC) AS RANK FROM TBL_SCORE) WHERE NAME = '이시우'
;
--==>> 1	이시우	80	70	240	80	1
...
WHERE SID = 1;
--> 한줄 구성
SELECT SID, NAME, ENG, MAT, TOT, AVG, RANK  FROM ( SELECT SID, NAME, KOR, ENG, MAT , (KOR + ENG + MAT) AS TOT , (KOR + ENG + MAT)/3 AS AVG , RANK() OVER(ORDER BY (KOR+ENG+MAT) DESC) AS RANK FROM TBL_SCORE ) WHERE SID = 1
;
--==>> 1	이시우	80	70	240	80	1
